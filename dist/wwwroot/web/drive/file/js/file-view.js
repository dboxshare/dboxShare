...
function viewInit() {
    ...
    $ajax({
        type: 'HEAD', 
        url: '/api/drive/file/view?id=' + id + '&extension=' + extension, 
        async: true, 
        callback: function(http) {
            if (http.status == 200) {
                if (type == 'msoffice' || type == 'openoffice' || type == 'wps' || type == 'pdf' || type == 'drawing') {
                    if (extension == '.pdf') {
                        viewPdf(url);
                    } else {
                        viewPdf(url + '.pdf');
...
function viewMedia(url) {
    ...
    if ($agent(/(iOS|iPhone|iPad|iPod|Android|Windows Phone|SymbianOS)/i) == false) {
        ...
        html += '<param name="movie" value="/libs/flvplayer.swf" />';
