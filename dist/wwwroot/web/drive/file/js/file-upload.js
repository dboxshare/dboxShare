...
function init() {
    ...
    $id('upload-tips').innerHTML = lang.file.tips['upload-note'].replace(/\{size\}/, window.top.myUploadSize);
}
...
jQuery(function() {
    ...
    var uploader = WebUploader.create({
        ...
        threads: 1, 
        fileNumLimit: 1024, 
        fileSingleSizeLimit: window.top.myUploadSize * 1024 * 1024, 
        duplicate: true, 
        fromData: {
            guid: '', 
            folderId: ''
    ...
    uploader.on('beforeFileQueued', function(file) {
        if (file.size > window.top.myUploadSize * 1024 * 1024) {
            fastui.textTips(lang.file.tips['upload-size-limit'].replace(/\{size\}/, window.top.myUploadSize));
            return false;
        }
    ...
    uploader.on('uploadFinished', function() {
        if (uploader.getStats().uploadFailNum == 0) {
            ...
            $id('file-list').innerHTML = '<div id="upload-tips">' + lang.file.tips['upload-note'].replace(/\{size\}/, window.top.myUploadSize) + '</div>';

            window.parent.uploadCompleteRefresh();
        }
        });
