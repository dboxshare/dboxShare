...
function dataReader() {
    $ajax({
        type: 'GET', 
        url: '/api/drive/task/inbox-data-json?id=' + $query('id'), 
        async: true, 
        callback: function(data) {
            if ($jsonString(data) == false) {
                ...
                $id('status').innerHTML = taskStatus(jsonData.ds_status);
                ...
                for (var i = 0; i < memberDataJson.items.length; i++) {
                    ...
                    row.addCell(
                        ...
                        function() {
                            ...
                            if (memberDataJson.items[i].ds_status == -1) {
                                if (memberDataJson.items[i].ds_userid == window.top.myId) {
