...
function init() {
    ...
    fastui.list.scrollDataLoad('/api/drive/task/outbox-list-json');
}
            ...
            function() {
                ...
                if (jsonData.items[i].ds_revoke == 0) {
                    if (new Date(jsonData.items[i].ds_deadline.replace(/\-/g, '/')).getTime() > new Date().getTime()) {
                        ...
                        html += '<span class="button-dark" onClick="fastui.windowPopup(\'task-outbox-detail\', \'' + lang.task.button['detail'] + ' - ' + jsonData.items[i].ds_filename + '' + jsonData.items[i].ds_fileextension + '\', \'/web/drive/task/task-outbox-detail.html?id=' + jsonData.items[i].ds_id + '\', 800, 500);">' + lang.task.button['processing'] + '</span>';
                    } else {
                        html += '<span class="warning">' + lang.task.data.status['expired'] + '</span>';
                        html += '<span class="button-light" onClick="fastui.windowPopup(\'task-outbox-detail\', \'' + lang.task.button['detail'] + ' - ' + jsonData.items[i].ds_filename + '' + jsonData.items[i].ds_fileextension + '\', \'/web/drive/task/task-outbox-detail.html?id=' + jsonData.items[i].ds_id + '\', 800, 500);">' + lang.task.button['detail'] + '</span>';
                    }
                } else {
