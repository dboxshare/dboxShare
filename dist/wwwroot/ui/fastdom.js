/*
 * fastdom js library v0.4.0.2104
...
/*
 * 超级元素选择器
 * @param {String} query 查找元素字符串(必需)
 *                       tag 根据标签名称获取元素集合
 *                       #id 根据 id 获取元素
 *                       .class 根据 class 获取元素集合
 *                       name=value 根据属性名称及值获取元素集合
 * @return {Element} 返回元素对象(集合)
 */
function $dom(element, query) {
    var elements = null;

    if (typeof(element) == 'string') {
        if (/^[\w\-]+$/i.test(element) == true) {
            elements = $tag(element, undefined);
        }

        if (/^\#[\w\-]+$/i.test(element) == true) {
            elements = $id(element.substring(1));
        }

        if (/^\.[\w\-]+$/i.test(element) == true) {
            elements = $class(element.substring(1), undefined);
        }

        if (/^[\w\-]+\s*\=\s*[\w\-]+$/i.test(element) == true) {
            var param = element.match(/^([\w\-]+)\s*\=\s*([\w\-]+)$/i);

            if (param[1] == 'name') {
                elements = $name(param[2], undefined);
            } else {
                elements = $attr(param[1], param[2], undefined);
            }
        }
    } else {
        if (/^[\w\-]+$/i.test(query) == true) {
            elements = $tag(element, query);
        }

        if (/^\.[\w\-]+$/i.test(query) == true) {
            elements = $class(element, query.substring(1));
        }

        if (/^[\w\-]+\s*\=\s*[\w\-]+$/i.test(query) == true) {
            var param = query.match(/^([\w\-]+)\s*\=\s*([\w\-]+)$/i);

            if (param[1] == 'name') {
                elements = $name(element, param[2]);
            } else {
                elements = $attr(element, param[1], param[2]);
            }
        }
    }
...
function $hide(element, time) {
    ...
    if (time == undefined || $html5() == false) {
        element.style.display = 'none';
    } else {
        element.style.filter = 'alpha(opacity=0)';
        element.style.opacity = '0';
        element.style.transition = 'all ' + time + 'ms ease';
        element.style.webkitTransition = 'all ' + time + 'ms ease';
        element.style.mozTransition = 'all ' + time + 'ms ease';
        element.style.msTransition = 'all ' + time + 'ms ease';
        element.style.oTransition = 'all ' + time + 'ms ease';
...
function $chain(element) {
    element.$dom = function(query) {
        return $dom(this, query);
        };
    ...
    element.$hide = function(time) {
        $hide(this, time);
        };

    element.$show = function(time) {
        $show(this, time);
        };
...
/*
 * 路径跳转或当前网址参数替换
 * @param {String} param 跳转路径或替换参数名称(必需)
