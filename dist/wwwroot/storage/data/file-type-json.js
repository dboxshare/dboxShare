var fileTypeJson = {};


fileTypeJson = {
    'word': 'doc,docm,docx,dot,dotm,dotx', 

    'excel': 'csv,xls,xlsb,xlsm,xlsx,xlt,xltm,xltx', 

    'powerpoint': 'pot,potm,potx,pps,ppsm,ppsx,ppt,pptm,pptx', 

    'project': 'mpp,mpt', 

    'publisher': 'pub', 

    'visio': 'vdw,vdx,vsd,vsdx,vss,vst,vsx,vtx', 

    'openoffice': 'odg,odp,ods,odt', 

    'wps': 'wps', 

    'pdf': 'pdf', 

    'text': 'rtf,txt', 

    'code': 'asp,aspx,bat,c,coffee,cpp,cs,css,go,htm,html,java,js,json,jsp,less,lua,m,php,pl,py,rb,sass,scss,sql,swift,ts,vb,vbs,xml', 

    'image': 'bmp,gif,jpeg,jpg,png,tif,tiff', 

    'drawing': '3ds,ai,blend,cdr,dae,dgn,dwg,dwf,dwfx,dxf,eps,fbx,max,plt,ply,psd,skp,stl,svg', 

    'audio': 'aac,flac,m4a,mp3,ogg,wav,wma', 

    'video': '3gp,asf,avi,flv,m4v,mkv,mov,mp4,mpeg,mpg,rm,rmvb,wmv', 

    'zip': '7z,rar,zip'
    };