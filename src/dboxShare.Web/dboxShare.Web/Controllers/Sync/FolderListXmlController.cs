...
namespace dboxShare.Web.Sync.Controllers
{
    ...
    public class Sync_FolderListXmlController : ApiController
    {
        ...
        public HttpResponseMessage FolderListXml()
        {
            ...
            try
            {
                ...
                var FolderList = new List<Hashtable>();

                var Sql = "Select DS_Id, DS_Folder, DS_FolderId, DS_Name, DS_Share, DS_Recycle From DS_File Where " + Query + " Order By DS_Name Asc, DS_Id Asc";
                ...
                {
                    Xml.Add("<item>");
                    Xml.Add("<id>" + FolderList[i]["DS_Id"].TypeString() + "</id>");
                    Xml.Add("<folderId>" + FolderList[i]["DS_FolderId"].TypeString() + "</folderId>");
                    Xml.Add("<name>" + FolderList[i]["DS_Name"].TypeString() + "</name>");
                    Xml.Add("</item>");
                }

                return AppCommon.ResponseMessage("<?xml version=\"1.0\" encoding=\"utf-8\"?><folders>" + string.Join("", Xml.ToArray()) + "</folders>");
            }
            finally
            {
                Conn.Close();
