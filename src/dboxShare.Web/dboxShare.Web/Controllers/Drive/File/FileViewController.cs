    ...
    public class FileViewController : ApiController
    {
        ...
        public HttpResponseMessage View_Get()
        {
            ...
            try
            {
                ...
                // 查看日志 20 分钟记录一次
                int FileViewed = Base.Data.SqlScalar("Select Count(*) From DS_Log Where DS_UserId = " + Context.Session["UserId"].TypeString() + " And DS_Action = 'file-view' And DS_Time > '" + DateTime.Now.AddMinutes(-20).ToString() + "' And DS_FileId = " + Id, ref Conn);
