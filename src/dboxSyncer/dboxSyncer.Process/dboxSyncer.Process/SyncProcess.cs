...
namespace dboxSyncer
{
    ...
    sealed class SyncProcess
    {
        ...
        public static void Main()
        {
            ...
            string Action = Args[1].TypeString();
        ...
        private static void TaskSchedule(string Action)
        {
            ...
            XmlNodeList XNodes = XDocument.SelectNodes("/rules/item");
            ...
            foreach (XmlNode XNode in XNodes)
            {
                ...
                string LocalFolderPath = XNode.SelectSingleNode("localFolderPath").InnerText;
                string NetFolderId = XNode.SelectSingleNode("netFolderId").InnerText;
                string NetFolderPath = XNode.SelectSingleNode("netFolderPath").InnerText;
                string Type = XNode.SelectSingleNode("type").InnerText;
        ...
        private static void NetDataToList(string NetFolderId, ref List<Hashtable> NetDataList, ref Hashtable NetFolderTable)
        {
            ...
            try
            {
                ...
                string Url = "" + AppConfig.ServerHost + "/api/sync/file-list-xml?folderid=" + NetFolderId + "&timestamp=" + DateTime.Now.ToString("yyyyMMddHHmmssfffffff") + "";

                byte[] ResponseData = WebClient.DownloadData(new Uri(Url));
            ...
            XmlNodeList XNodes = XDocument.SelectNodes("/root/folder");
        ...
        private static void FileUpload(string FilePath, string FileSize, string FileHash, string FolderId)
        {
            ...
            try
            {
                ...
                string UploadUrl = "" + AppConfig.ServerHost + "/api/sync/file-upload?guid=" + System.Guid.NewGuid().ToString() + "&folderid=" + FolderId + "&filepath=" + HttpUtility.UrlEncode(FilePath) + "&filesize=" + FileSize + "&filehash=" + FileHash + "&timestamp=" + DateTime.Now.ToString("yyyyMMddHHmmssfffffff") + "";

                using (Stream WebStream = WebClient.OpenWrite(UploadUrl))
                {
                    using (FileStream FStream = new FileStream(FilePath, FileMode.Open, FileAccess.Read, FileShare.ReadWrite, 4096, true))
                    {
        ...
        private static void FileUpversion(string FilePath, string FileSize, string FileHash, int FileId)
        {
            ...
            try
            {
                ...
                string UpversionUrl = "" + AppConfig.ServerHost + "/api/sync/file-upversion?guid=" + System.Guid.NewGuid().ToString() + "&fileid=" + FileId + "&filepath=" + HttpUtility.UrlEncode(FilePath) + "&filesize=" + FileSize + "&filehash=" + FileHash + "&timestamp=" + DateTime.Now.ToString("yyyyMMddHHmmssfffffff") + "";

                using (Stream WebStream = WebClient.OpenWrite(UpversionUrl))
                {
                    using (FileStream FStream = new FileStream(FilePath, FileMode.Open, FileAccess.Read, FileShare.ReadWrite, 4096, true))
                    {
                        byte[] Buffer = new byte[65536];
        ...
        private static void FileDownload(int FileId, string FileCodeId, string FileName, string LocalFolderPath)
        {
            ...
            string TempFilePath = AppCommon.PathCombine(AppConfig.AppPath, "temp", FileName);

            string SaveFilePath = AppCommon.PathCombine(LocalFolderPath, FileName);
            ...
            try
            {
                ...
                string DownloadUrl = "" + AppConfig.ServerHost + "/api/sync/file-download?id=" + FileId + "&codeid=" + FileCodeId + "&timestamp=" + DateTime.Now.ToString("yyyyMMddHHmmssfffffff") + "";

                using (Stream WebStream = WebClient.OpenRead(DownloadUrl))
                {
                    using (FileStream FStream = new FileStream(TempFilePath, FileMode.Create, FileAccess.Write, FileShare.ReadWrite, 4096, true))
                    {
                        byte[] Buffer = new byte[65536];
                        int Read = 0;
        ...
        private static void SyncLog(string Action, string FilePath)
        {
            ...
            string[] Files = Directory.GetFiles(AppCommon.PathCombine(AppConfig.AppPath, "log"));
